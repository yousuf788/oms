...
- On startup, both `order-process` (from `last_committed_order_id+1` — read from its own WAL via `LeaderElection::max_committed_order_id()`) and `order-receiver` (from `checkpoint+1`) fire one unconditional catch-up request for `..u64::MAX`, instead of only detecting a gap reactively once a new live order happens to arrive. order-sending never stops sending on its own, so this makes recovery start immediately on process start rather than waiting on the next live order's arrival to reveal how far behind the node is.
