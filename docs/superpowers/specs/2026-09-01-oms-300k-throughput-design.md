> **Status (2026-09-03)**: historical planning document — kept as a record of what was designed
> and why, not a live spec. The bincode/Aeron/HMAC migration described here has been implemented
> (see `docs/BENCHMARK.md` for the current build). The 300k orders/sec target itself has **not**
> been validated on real lab hardware — see `docs/BENCHMARK.md` §0 for what has actually been
> measured (a single-machine simulation, not the 3-machine deployment this spec assumes) and
> `docs/HLD.md` §7 for the sequencing/gap-detection/replay protocol built after this spec was
> written, which this document does not cover.
